...
describe('useAnalyticsEvent - Integration Tests', () => {
  ...
  describe('Multiple component instances', () => {
    it.skip('should share state between multiple component instances', async () => {
  ...
  describe('Real-world scenarios', () => {
    ...
    it('should handle user identification flow', async () => {
      ...
      // Set user ID
      wrapper.vm.analytics.set('user_id', 'user_12345')
      await nextTick()
    ...
    it.skip('should handle consent management flow', async () => {
      const wrapper = mount(TestComponent)
    ...
    it.skip('should handle analytics reset during session', async () => {
      const wrapper = mount(TestComponent)
      const { defineAnalyticsEvent } = await import('../../src/runtime/utils')

      // Send some events
      wrapper.vm.analytics.sendEvent(defineAnalyticsEvent('event_1', {}))
      wrapper.vm.analytics.sendEvent(defineAnalyticsEvent('event_2', {}))
      nextTick()
  ...
  describe('Error resilience', () => {
    it('should continue working after failed event send', async () => {
      ...
      wrapper.vm.analytics.sendEvent(null as unknown)

      // Should still work for valid events
      const validEvent = defineAnalyticsEvent('valid_event', {})
      const result = wrapper.vm.analytics.sendEvent(validEvent)
    ...
    it('should handle window.dataLayer being deleted mid-session', async () => {
      ...
      // Should not throw
      expect(() => wrapper.vm.analytics.reset()).not.toThrow()
